...
public class EspacoDAO {
    ...
    public void cadastrar(Espaco e) {
        String sql = "INSERT INTO espaco (nome, descricao, capacidade, valor_hora, status) VALUES (?, ?, ?, ?, ?)";
        try (Connection con = ConnectionFactory.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {

            ps.setString(1, e.getNome());
            ps.setString(2, e.getDescricao());
            ps.setInt(3, e.getCapacidade());
    ...
    public void atualizar(Espaco e) {
        String sql = "UPDATE espaco SET nome=?, descricao=?, capacidade=?, valor_hora=?, status=? WHERE id=?";
        try (Connection con = ConnectionFactory.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
