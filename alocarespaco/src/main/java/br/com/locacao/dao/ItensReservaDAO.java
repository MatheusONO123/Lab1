...
public class ItensReservaDAO {
    ...
    public void cadastrar(ItensReserva item) {
        String sql = "INSERT INTO itens_reserva (reserva_id, servico_id, quantidade, valor_unitario) VALUES (?, ?, ?, ?)";
        try (Connection con = ConnectionFactory.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {

            ps.setInt(1, item.getReservaId());
            ps.setInt(2, item.getServicoId());
    ...
    public List<ItensReserva> listarPorReserva(int reservaId) {
        ...
        String sql = "SELECT ir.*, s.nome AS servico_nome " +
                     "FROM itens_reserva ir " +
                     "JOIN servico s ON ir.servico_id = s.id " +
                     "WHERE ir.reserva_id = ?";
        try (Connection con = ConnectionFactory.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
